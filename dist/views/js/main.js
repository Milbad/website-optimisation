...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    //create random variable to store the DOM element
    //use getElementByClassName instead of querySelector
    var random= document.getElementsByClassName("randomPizzaContainer");
    //move dx and newwidth outside of the loop
     var dx = determineDx(random[0], size);
      var newwidth = (random[0].offsetWidth + dx) + 'px';
...
// https://www.igvita.com/slides/2012/devtools-tips-and-tricks/jank-demo.html
  //Move items outside of the updatePosition function
  var items = document.getElementsByClassName('mover');

// Moves the sliding background pizzas based on scroll position
function updatePositions() {
  frame++;
  //Move variables phase, i and l outside of the  loop
  //create a variable top to store document.body.scrollTop outside of the for loop
  var i;
  var l;
  var phase=[];
  window.performance.mark("mark_start_frame");
  var top = document.body.scrollTop/1250;
  //create 2 loops. 1 to calculate phase and the other to update the style of each item
  for(l=0;l<5;l++){
    phase.push(Math.sin(top + l)*100);
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  //Declare the variable elem outside of the for loop
  var elem;
  var screenHeight = window.innerHeight;
  var numPizza= screenHeight/s*cols;
  //Reduce the number of sliding pizzas regarding the height of the window
